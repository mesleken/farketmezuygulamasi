...
// Core 4-5 High-Information Dimensions from Onboarding
...
// Rejection Reason Tags with distinct persistence scopes
...
export type PersistenceScope = 'persistent_profile' | 'daily_context' | 'session_context';
...
export interface SurpriseRequest {
  ...
  targetDateTime: string; // e.g. "Cuma 19:30"
  budgetLevel?: BudgetLevel;
  stylePreference?: 'romantic' | 'active_adventurous' | 'cozy_dinner';
  venueId: string;
  venue: VenueCandidate;
  receiverTeaser: string; // e.g. "Cuma 19:30'da hazır ol! Konum gizli 🤫"
...
export interface CoupleFairnessScore {
  coupleId: string;
  userId: string;
  satisfactionScore: number; // 0.0 to 1.0
  isManualOverride: boolean; // if user pressed "bu sefer sen seç", does not skew auto-balance
  timestamp: string;
}
...
export interface CoupleProfile {
  ...
  surpriseAffinities: Record<string, number>; // Specific learning channel for surprises, separate from PreferenceWeight
